...
text_input_context::ProcessKey(os_key_code Key){
    if(Key == KeyCode_NULL){
        ...
        if((Char == 'C') && Input->TestModifier(KeyFlag_Control|KeyFlag_Any)){ //- Copy
            ...
            if(Input->TestModifier(KeyFlag_Shift|KeyFlag_Any)) Char = KEYBOARD_SHIFT_TABLE[(u32)Char]; 
            Assert(Char);
...
SocketSendCString(os_socket *Socket, const char *S){
    return SocketSendData(Socket, S, CStringLength(S)+1);
}

internal socket_send
SocketSendBuilderData(os_socket *Socket, string_builder *Builder){
    return SocketSendData(Socket, Builder->Buffer, Builder->BufferSize);
}
