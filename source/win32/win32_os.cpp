...
ConnectSocket(const char *HostName, u32 Port){
    ...
    addrinfo *OriginalInfo = 0;
    {
        ...
        DWORD Error = getaddrinfo(HostName, PortString, 0, &OriginalInfo);
        if(Error){
            int A = 0;
            return Result;
        }
    ...
    if(OriginalInfo && OriginalInfo->ai_addr){
        ...
        SOCKET Sock = socket(Info->ai_addr->sa_family, SOCK_STREAM, IPPROTO_TCP);
...
ClientSocket(os_socket Listen){
    ...
    SOCKET Sock = accept(Listen.ID, 0, 0);
...
SocketSendCString(os_socket *Socket, const char *S){
    socket_send Result = {};
    if(!Socket->Valid) return Result;
    
    s32 Length = CStringLength(S);
    int BytesSent = send(Socket->ID, S, Length+1, 0);
    Result.BytesSent = BytesSent;
    if(BytesSent != SOCKET_ERROR){
        Result.Success = true;
        if(Length+1 == BytesSent){
            Result.SentAll = true;
        }
    }
